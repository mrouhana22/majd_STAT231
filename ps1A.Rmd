...
> ANSWER: `reorder(drg, mean_charge)` sorts the medical procedures in order of their average charges, from least expensive to most expensive. By using `x = drg` instead, the plot becomes out of order of expenses, and instead is in order of the DRG number.
...
> ANSWER: `theme(axis.text.x = element_text(angle = 90, hjust = 1))` rotates the DRG numbers text on the x-axis by 90 degrees so that they are read sideways. This makes it so that the numbers don't overlap each other, improving readability of the graph.
...
> ANSWER:\newline
Question 1: What does the `with()` function do? (Page 52, second code chunk) \newline
Question 2: What is the purpose of the `arrow` argument? (Page 53, first code chunk)
