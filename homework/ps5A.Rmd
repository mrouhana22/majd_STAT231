...
> ANSWER: In the first case, `grepl()` searches the first six indices and returns `TRUE` if the index contains " MACBETH" and `FALSE` if it doesn't (it returns six `FALSE`s because " MACBETH" doesn't occur until the 228th index). In the second case, we add `value = TRUE` to `grep()`, so it returns the first six strings in the `macbeth` dataset that include " MACBETH". In the third case, `grep()` returns the first six indices of the `macbeth` dataset where the string " MACBETH" was found.
...
> ANSWER: The first line returns the first six strings where "MACBETH." is found. The second line returns the first six strings where "MACBETH" is found (not necessarily with a period). This is because `.` is a metacharacter that matches any character. If we want to specify to search for `.` as well, we must escape it with a backslash (as shown in the first line).
...
> ANSWER: The same results are returned for each different pair. The pattern "MAC[B-Z]" indicates that R is searching for any lines that contain "MAC" followed by any capital letter other than "A".
