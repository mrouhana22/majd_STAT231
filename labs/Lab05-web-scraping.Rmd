...
> ANSWER: It starts with `https://en.wikisource.org/wiki/` followed by the title of the poem separated by underscores.
...
dickinson <- url %>%
  read_html() %>%
  html_nodes("table")
dickinson <- html_table(dickinson[[1]])
dickinson
```
...
```{r}
as.data.frame(dickinson)
dickinson <- mutate(dickinson, URL = paste0("https://en.wikisource.org/wiki/",
                                            str_replace_all(dickinson[, 1], " ", "_")))
```
