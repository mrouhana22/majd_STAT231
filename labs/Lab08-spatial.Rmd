...
In this lab we will work with the some familiar packages and some new packages.  There primary new packages we'll be working with for mapping is: 
...
(1) identify a data source that provides the correct spatial information needed (this may be in the form of a shapefile, or using the `maps` package, for instance)
...
As an example, let's consider data collected by The Chronicle for Higher Education on Colleges' reopening plans for Fall 2020.  The data file we have was downloaded in July 2020, so reflect the institutions' reopening plans as of July 2020.

Source: https://www.chronicle.com/article/heres-a-list-of-colleges-plans-for-reopening-in-the-fall

Let's make a map of the US, with each state colored by the proportion of institutions in the state that were planning to be in person this Fall as of July.
...
- Copy your image onto the Jamboard [here](https://jamboard.google.com/d/1su9zZWYZxHLcOxj2p42dyKjya-0PCrQVnDQTfd8619o/edit?usp=sharing)
...
usMap <- merge(college_plans_map, hate_crimes) %>%
  select(state, long, lat, share_non_white, group) %>%
  distinct()
...
ggplot(usMap, aes(x = long, y = lat, group = group
  ...
  theme_void() +
  coord_fixed(ratio = 1.3) +
  labs(title = "Share of each state's population that is not white, 2015"
       , subtitle = "Majd Rouhana"
       , caption = "* States in gray have no data"
       , fill = "") +
  scale_fill_viridis(option = "magma", direction = -1) 
