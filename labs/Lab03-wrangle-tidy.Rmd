...
> RESPONSE: The years 1991-2014 are covered in the `spending` dataset. The years 2013-2016 are covered in the `coverage` dataset. I confirmed this in the dataset by looking at the variable names. The variables are labeled by their year.
...
> RESPONSE: We need to include the total spending costs variable and the employer coverage variable. The rows in the dataset represent a state and year in the US. For example, Alabama 2013.


What are some of the steps that will need to be taken to get the data in that form?  

> RESPONSE: We can filter out the variables that we do not need to include. We can also transform the variables by multiplying them by log10.
...
```{r}
# creating final coverage dataset
coverage4 <- coverage3 %>%
  select(Location, "2013" = "2013__Employer", "2014" = "2014__Employer") %>%
  gather(key = "Year", value = "Employer", "2013", "2014")

# creating final spending dataset
spending4 <- spending3 %>%
  select(Location, "2013" = "2013__Total", "2014" = "2014__Total") %>%
  gather(key = "Year", value = "Spending", "2013", "2014")

covSpend <- merge(coverage4, spending4)

# plot the data
ggplot(data = covSpend
       , mapping = aes(x = Spending, y = Employer, color = Year)) +
    geom_point() +
    geom_smooth(method = "loess", se = FALSE) +
    labs(
      title = "Total Spending Costs vs Employer Coverage",
      subtitle = "2013-2014",
      y = "Employer Coverage",
      x = "Total Spending Costs",
      color = "Year"
    )
```
